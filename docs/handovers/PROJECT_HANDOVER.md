...
**Last updated:** 2026-08-23, immediately after the repository cleanup described in §6.
...
**V2's only dependency on V1 is `src/baseline/`** — the frozen classical first-order estimator
...
`src/baseline/`; Phase 2A–2F `*_results.json`; condition definitions F0–F4; seeds (0);
...
| `__pycache__/`, empty `v2_research/{data,experiments,literature,scripts}/` | Generated clutter / empty directories. |
...
makes each claim. `v2_research/data_audit/` is kept as human-readable dataset provenance.
...
```
README.md                     project overview, findings, reproduction, dataset placement
requirements.txt
.gitignore                    excludes both datasets and all generated parquet/CSV
docs/handovers/
  PROJECT_HANDOVER.md         this file
src/baseline/                 frozen classical estimator (only V1 module V2 needs)
v2_research/
  README.md                   V2 pipeline guide
  paper/                      manuscript.md, abstract, figures/, tables/, references/, claims/
  paper_analysis/             run_all.py, validate_results.py, verify_artifacts.py,
                              make_tables.py, make_figures.py, load_results.py, config.py,
                              paper_results_manifest.json, figures/, tables/, evidence docs
  summit/                     Phase 2A-2F generators + locked JSON results + provenance
  data_audit/                 dataset provenance
```

Roughly 4 MB committed across ~180 files.
...
|---|---|---|---|
| Summit power/thermal (DOI `10.13139/OLCF/1861393`, CC-BY-4.0) | ~12 GB | `v2_research/summit/raw/`, derived to `derived/cleaned/` | **No** |
...
1. **Submit the manuscript.** `v2_research/paper/manuscript.md`. Target **FGCS** or **JPDC**
