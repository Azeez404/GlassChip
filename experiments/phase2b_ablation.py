...
from glasschip.models import ClassicalBaselineModel  # noqa: E402  (frozen, read-only)
...
def pilot_hosts(n):
    hs = []
    for mf in sorted(_REPO / "artifacts/manifests/per_host".glob("*.json")):
...
def main():
    ...
    for cname, cond in CONDITIONS.items():
        ...
        (OUT / "phase2b_ablation.json").write_text(json.dumps(results, indent=2, default=str))
    ...
    # CSV table
    import csv
    with open(OUT / "phase2b_ablation_table.csv", "w", newline="") as f:
