...
from glasschip.models import ClassicalBaselineModel  # noqa: E402  (V1 frozen, read-only)

_REPO = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(_REPO / "src"))
sys.path.insert(0, str(Path(__file__).resolve().parent))

DERIVED = _REPO / "data/summit/derived"
...
def main() -> int:
    ...
    ckpt = out / "phase2a_counterfactual.json"
    ...
    out = DERIVED.parent / "counterfactual"
    out.mkdir(parents=True, exist_ok=True)
    (out / "phase2a_counterfactual.json").write_text(json.dumps(results, indent=2, default=str))
